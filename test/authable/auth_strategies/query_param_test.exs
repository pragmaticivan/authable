defmodule Authable.AuthStrategy.QueryParamTest do
  ...
  test "returns user model when authenticates with access_token query string using valid data",
       %{
         conn: conn
       } do
    user = insert(:user)
    ...
    token =
      insert(
        :access_token,
        user: user,
        details: %{client_id: client.id, scope: "read,write"}
      )
  ...
  test "returns :error when fails to authenticates with access_token query string using invalid data",
       %{conn: conn} do
