defmodule Authable.Authentication.BasicTest do
  ...
  test "authorize with malformed authentication hash" do
    ...
    expected = {:error, %{invalid_request: "Invalid credentials encoding.",
      headers: [%{"www-authenticate" => "Basic realm=\"authable\""}]},
      :bad_request}
  ...
  test "error when non existent email" do
    ...
    expected = {:error, %{invalid_credentials: "Identity not found.",
      headers: [%{"www-authenticate" => "Basic realm=\"authable\""}]},
      :unauthorized}
    assert expected == result
  end
