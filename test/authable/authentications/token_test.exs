defmodule Authable.Authentication.TokenTest do
  ...
  setup do
    ...
    insert(:access_token, %{value: @expired_token_value, user: user,
      expires_at: :os.system_time(:seconds) - 1000})
    insert(:access_token, %{value: @access_token_value, user: user})
    insert(:access_token, %{value: @deleted_user_token_value,
      user: second_user})
  ...
  test "authorize with bearer token" do
    authorized_user =
      case TokenAuthentication.authenticate({"access_token",
        @access_token_value}, []) do
        {:error, _, _} -> nil
        user -> user
      end
  ...
  test "error when expired token given" do
    {:error, _, status} = TokenAuthentication.authenticate({"access_token",
      @expired_token_value}, [])
  ...
  test "authorize with session token" do
    authorized_user =
      case TokenAuthentication.authenticate({"session_token",
        @session_token_value}, []) do
        {:error, _, _} -> nil
        user -> user
      end
    refute is_nil(authorized_user)
